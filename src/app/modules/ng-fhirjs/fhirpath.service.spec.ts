...
describe('FhirPathService', () => {
  ...
  describe('#simplefhirpathtests', () => {
    ...
    it('CapabilityStatement.status', () => {
      expect(service.evaluate(resource, 'CapabilityStatement.status')[0]).toBe('active');
    });

    it('CapabilityStatement.date', () => {
      expect(service.evaluate(resource, 'CapabilityStatement.date')[0]).toBe('2018-01-03T17:31:22+01:00');
    });

    it('CapabilityStatement.software.name', () => {
      expect(service.evaluate(resource, 'CapabilityStatement.software.name')[0]).toBe('HAPI FHIR Server');
    });
