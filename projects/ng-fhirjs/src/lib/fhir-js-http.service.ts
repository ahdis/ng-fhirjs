...
import fhirjs from 'fhir.js/src/fhir';

export const FHIR_HTTP_CONFIG = new InjectionToken<FhirConfig>(
  'fhir configuration'
);
...
export class FhirJsHttpService {
  ...
  constructor(
    ...
    @Inject(FHIR_HTTP_CONFIG) config: FhirConfig
  ) {
  ...
  private getService(httpClient: HttpClient, config: FhirConfig): IFhir {
    ...
    function getResponse(
      response: HttpResponse<IResource>,
      args: RequestObj
    ): ResponseObj {
    ...
    const adapter: Adapter = {
      http: (args: RequestObj): Promise<ResponseObj> => {
        ...
        return new Promise((resolve, reject) => {
          ngHttpClient
            .request<IResource>(args.method, url, {
              body,
              headers,
              observe: 'response',
            })
